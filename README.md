...
| 量化选股 | **16 因子**打分 + **13 套预设策略**（动量核心 / 52周突破 / 放量启动 / 波动压缩 / 超卖反弹 / 小盘高弹性 / 算力链轮动 …），每个预设都写明交易逻辑与**最容易亏在哪**。扫描规模 7 档：**50 / 100 / 150 / 200 / 300 / 500 / 1000 只**，1000 只实测约 40 秒，扫描中可取消，结果分页浏览 |
...
量化选股支持 7 档规模：**50 / 100 / 150 / 200 / 300 / 500 / 1000 只**。

规模越大，并发越高、请求间隔越短（数据源有限流，参数是按实测调的）：
...
1. **档位数量从主进程取**。前端不硬编码 —— 否则迟早出现「前端能选 1000、后端只认 300」。
2. **候选池自动补齐**。内置池（标普 500 权重股 70 只）不足时用全市场成交额榜补齐，
   并在结果里标注「其中 N 只来自全市场补齐」。**不会**静默只扫 70 只却让你以为扫完了全市场。
3. **排行榜自带市值与 PE，直接复用**，不再逐只补行情。1000 只扫描原本有一半时间耗在这一步。
4. **可取消**。扫到一半发现选错池子，点取消即停，已完成的部分保留。
5. **结果分页**（每页 100 行）。1000 行一次性塞进 DOM 会让滚动和列排序明显卡顿；
   排序与筛选仍作用于全量，导出 CSV 也是全量。
6. **限流参数用完就恢复**。否则一次激进扫描会把全局参数永久留在高并发档，
   之后所有请求都贴着限流边缘跑。

压测脚本：`npm run scan-load -- 1000`
...
到本仓库的 **[Releases](../../releases)** 页面下载 `QuantDesk-1.2.0-arm64.dmg`，双击挂载，把 QuantDesk 拖入 Applications。
